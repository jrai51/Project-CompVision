...
def visualize_depth_sample(
    ...
    # -------------------------
    # 4) Unnormalize RGB for visualization
    # -------------------------
    ...
    # -------------------------
    # 6) Compute metrics (masked)
    # -------------------------
    valid_pred = pred_np[gt_mask_np]
    valid_gt = gt_np[gt_mask_np]

    mse = ((valid_pred - valid_gt) ** 2).mean().item()
    rmse = mse ** 0.5
    ...
    if rmse == 0:
        ...
        psnr = 20 * torch.log10(torch.tensor(vmax / rmse)).item()
    ...
    im_sparse = axs[0, 1].imshow(sparse_vis, vmin=0, vmax=vmax, cmap="magma")
    axs[0, 1].set_title("Sparse LiDAR")
    axs[0, 1].axis("off")
    fig.colorbar(im_sparse, ax=axs[0, 1], fraction=0.046, pad=0.04)

    im_mono = axs[0, 2].imshow(mono_vis, cmap="magma")
...
if __name__ == "__main__":
    ...
    if model_type == "lidar_only":
        ...
        model = DepthUNetMonoFusion(base_ch=32, residual=False)
